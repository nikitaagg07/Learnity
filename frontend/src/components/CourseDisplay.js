...
import {
  ...
  IconButton, CircularProgress, CardMedia, Divider,
...
const CourseDisplay = () => {
  ...
  const [isEnrolled, setIsEnrolled] = useState(false); // State to store enrollment status
  ...
  const learnerId = localStorage.getItem('learnerId') || sessionStorage.getItem('learnerId');
  ...
  useEffect(() => {
    ...
    const fetchLearnerData = async (learnerId, courseId) => {
      try {
        const url = `http://localhost:5000/api/learners/me?learnerId=${learnerId}`;
        ...
        const CoursesArr = data?.data?.learners?.courses || [];
    
        const isEnrolledInCourse = CoursesArr.includes(courseId);
        setIsEnrolled(isEnrolledInCourse); // Update state based on enrollment
    ...
    const fetchCourseData = async () => {
      try {
        const url = `http://localhost:5000/api/courses/${courseId}${learnerId ? `?learnerId=${learnerId}` : ''}`;
        const response = await fetch(url);
      ...
      // Now check if learner is enrolled
      if (learnerId) {
        fetchLearnerData(learnerId, courseId);
      }

        
        // // Check if the learner is enrolled in this specific course
        // if (data.curriculum && data.curriculum.length > 0) {
        //   // Check if any lesson in the curriculum has isEnrolled set to true
        //   // or check if there's an enrollment field in the course data
        //   const learnerEnrolled = data.curriculum.some(lesson => 
        //     lesson.isEnrolled === true
        //   ) || data.isEnrolled === true;
          
        //   setIsEnrolled(learnerEnrolled);
        // }
      } catch (error) {
  ...
  const translateText = async () => {
    ...
    try {
      ...
      const response = await axiosInstance.post('http://127.0.0.1:5000/translate', {
  ...
  const prefetchAudio = useCallback(async (text, lang = 'en') => {
    ...
    try {
      const response = await axiosInstance.post('http://127.0.0.1:5000/speak', {
        text,
        language: lang
      });
  ...
  // const isEnrolledT = courseDetails.curriculum.some(lesson => lesson.freePreview);..  const audioCache = useRef({});
  

  return (
    <Box sx={{ maxWidth: 1200, margin: "auto", padding: 3, backgroundColor: "#f9f9f9", paddingY: 5 }}>
      <Card sx={{ mb: 4, backgroundColor: "#263238", color: "white", borderRadius: 2 }}>
        <CardContent>
          <Typography variant="h4" fontWeight="bold">
            {translatedData?.title || courseDetails.title}
          </Typography>
          <Typography variant="h6">
            {translatedData?.subtitle || courseDetails.subtitle}
          </Typography>
          ...
          <Box sx={{ mt: 3, display: "flex", alignItems: "center", flexWrap: "wrap", gap: 2 }}>
            ...
            >
              {isTranslating ? <CircularProgress size={24} color="inherit" /> :
  ...
  {/* Course Curriculum Section */}
            ...
            <CardContent sx={{ textAlign: "center" }}>
              ...
              <Typography variant="h5" fontWeight="bold">
                ...
                <Button
                  ...
                  fullWidth
                  sx={{ mt: 2 }}
                  onClick={() => navigate(`/video/${courseId}/0`)}
                >
                  Continue Learning
                </Button>
      ...
      <Dialog open={showPreviewDialog} onClose={() => setShowPreviewDialog(false)} maxWidth="md" fullWidth>
        ...
        <DialogActions>
          <Button onClick={() => setShowPreviewDialog(false)} color="secondary">
            Close
          </Button>
        </DialogActions>
      </Dialog>
